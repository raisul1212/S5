#!/bin/bash -l
# Corner 3' config (Mambino P=8 r=40 iso-param 189K) + chip eval sweep.
#SBATCH --job-name=corner3p
...
#SBATCH --output=results/slurm/corner3p_%j.out
#SBATCH --error=results/slurm/corner3p_%j.err
...
CKPT_DIR="./checkpoints/corner3p_seed${SEED}_${JOB}"
mkdir -p "$CKPT_DIR"

echo "Corner 3' (Mambino half_glu2 r=40 P=8) seed=$SEED (JOB=$JOB, SHA=$SHA)"
