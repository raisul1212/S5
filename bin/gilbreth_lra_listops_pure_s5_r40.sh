#!/bin/bash -l
# Pure S5 P=8 with glu_rank=40 -- mid-decay sweep point.
# Expected params: ~139K.  Phase 1 of the rank decay curve.
#SBATCH --job-name=s5-pure-r40
...
#SBATCH --output=results/slurm/s5_pure_r40_listops_%j.out
#SBATCH --error=results/slurm/s5_pure_r40_listops_%j.err
...
CKPT_DIR="./checkpoints/pure_s5_r40_${JOB}"
mkdir -p "$CKPT_DIR"

echo "Pure S5 P=8 glu_rank=40 LRA-ListOps sweep (JOB=$JOB, SHA=$SHA)"
