#!/bin/bash -l
# Mambino-in-S5 P=8 with glu_rank=64 factorized -- rank-constraint control.
# Params: ~238K (SAME as Corner 4 Mambino full-rank via nn.Dense).
# Matches Pure S5 r=64 factorized for the direct isolation experiment.
#SBATCH --job-name=s5-mamb-r64
...
#SBATCH --output=results/slurm/s5_mambino_r64_listops_%j.out
#SBATCH --error=results/slurm/s5_mambino_r64_listops_%j.err
...
CKPT_DIR="./checkpoints/mambino_r64_${JOB}"
mkdir -p "$CKPT_DIR"

echo "Mambino-in-S5 P=8 glu_rank=64 factorized LRA-ListOps (JOB=$JOB, SHA=$SHA)"
