#!/bin/bash -l
# Pure S5 activation=gelu (NO gate, 56K params) + chip eval sweep.
#SBATCH --job-name=chip-pure-gelu
...
#SBATCH --output=results/slurm/chip_pure_gelu_%j.out
#SBATCH --error=results/slurm/chip_pure_gelu_%j.err
...
CKPT_DIR="./checkpoints/chip_pure_gelu_${JOB}"
mkdir -p "$CKPT_DIR"

echo "Pure S5 gelu + chip eval (JOB=$JOB, SHA=$SHA)"
