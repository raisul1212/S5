...
#SBATCH --output=results/slurm/config3_%j.out
#SBATCH --error=results/slurm/config3_%j.err
...
CKPT_DIR="./checkpoints/config3_seed${SEED}_${JOB}"
mkdir -p "$CKPT_DIR"

echo "Config 3 (Pure S5 gelu P=8) seed=$SEED (JOB=$JOB, SHA=$SHA)"
